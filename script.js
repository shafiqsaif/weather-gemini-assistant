...
// ==============================
// AI Weather Assistant (SECURE)
// Frontend calls Azure/Netlify backend endpoints:
...
// Main function to get data (NOW from /api/weather)
async function executeSearch() {
  const city = cityInput.value.trim();
  if (!city) return showError("Please enter a valid city name.");
  ...
  loadingIndicator.classList.remove('d-none');
  document.getElementById('aiSummaryDisplay').innerHTML =
    '<div class="spinner-border spinner-border-sm text-light" role="status"></div> Thinking...';

  try {
    // ✅ Call YOUR backend (Azure Static Web Apps Functions OR Netlify Functions)
    const res = await fetch(`/api/weather?city=${encodeURIComponent(city)}`);
    const payload = await res.json();

    if (!res.ok) throw new Error(payload.error || "Failed to fetch weather data.");

    const { weatherData, forecastData, aqiData } = payload;
    ...
    // Ask AI for advice (NOW via /api/ai)
    fetchAIInsights(weatherData, aqiData);
    ...
    showError(error.message);
    document.getElementById('aiSummaryDisplay').innerHTML =
      `<p><em>AI is waiting because weather data failed.</em></p>`;
  }
}
...
function populateWeatherUI(data, aqiData) {
  ...
  const utcTime = new Date().getTime() + (new Date().getTimezoneOffset() * 60000);
  const localCityTime = new Date(utcTime + (data.timezone * 1000));
  document.getElementById('localTimeDisplay').innerHTML =
    `<i class="bi bi-clock"></i> Local Time: ${localCityTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;

  // Fixing sunrise and sunset time
  const formatTime = (unixTime) =>
    new Date(utcTime + (data.timezone * 1000) + ((unixTime - data.dt) * 1000))
  ...
  const aqiElement = document.getElementById('aqiDisplay');
  aqiElement.innerText = aqiLabels[aqiIndex] || "Unknown";
  aqiElement.className = `text-white mb-0 ${aqiColors[aqiIndex] || ""}`;
}
...
// AI Insights (NOW from /api/ai)
async function fetchAIInsights(weatherData, aqiData) {
  const aqiLabels = ["", "Excellent", "Fair", "Moderate", "Poor", "Hazardous"];
  const aqiIndex = aqiData.list?.[0]?.main?.aqi ?? 0;
  const aqiText = aqiLabels[aqiIndex] || "Unknown";

  const prompt = `
The weather in ${weatherData.name} is ${weatherData.main.temp}°C, feels like ${weatherData.main.feels_like}°C,
with ${weatherData.weather[0].description}. Air quality is ${aqiText}.
...
`.trim();
  ...
  try {
    // ✅ Call your backend (this hides GROQ_KEY)
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || "AI Insights failed.");

    const cleanText = sanitizeAIOutput(result.content);
    document.getElementById('aiSummaryDisplay').innerHTML = cleanText;
    ...
    document.getElementById('aiSummaryDisplay').innerHTML = `
      <div class="p-3 bg-danger bg-opacity-25 border border-danger rounded text-white">
        ...
        AI Insights currently unavailable: ${error.message}
      </div>`;
  }
}
...
function sanitizeAIOutput(rawText) {
  if (!rawText) {
    return "<p><em>AI returned an empty response. Please analyze the city again.</em></p>";
  }

  let formattedText = String(rawText).replace(/```html/g, '').replace(/```/g, '').trim();
...
function showError(message) {
  errorDisplay.innerHTML =
    `<i class="bi bi-shield-exclamation"></i> <strong>System Alert:</strong> ${message}`;
  errorDisplay.classList.remove('d-none');
